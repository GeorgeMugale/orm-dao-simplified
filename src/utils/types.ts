...
export interface Clause<T extends Entity> {
   ...
   */
  value: T[keyof T] | any[] | null; // Use `any[]` for 'IN' array values
}
...
export type WhereExpression<T extends Entity> =
  | WhereCondition<T>
  | WhereComposite<T>;
...
export type Join<L extends Entity, R extends Entity> = {
   ...
   */
  rightTable: (new (...args: any[]) => R) & typeof Entity; // Stricter typing for class reference
...
export type Options<T extends Entity> = {
   ...
   */
  joins?: Join<any, any>[]; // Keep `any, any` here to allow an array of mixed joins
  /**
